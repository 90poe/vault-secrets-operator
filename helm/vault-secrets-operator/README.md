...
[vault-secrets-operator](https://github.com/90poe/vault-secrets-operator) is Secrets controller for Kubernetes using secrets from Vault
...
To use, add secrets to Vault and create VaultSecrets CRD object in K8S.

This chart bootstraps an vault-secrets-operator deployment on a [Kubernetes](http://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.

## Prerequisites

- Chart version 1.x.x: Kubernetes v1.22+

## Get Repo Info

```
git clone git@github.com:90poe/vault-secrets-operator.git
```
...
```console
cd helm/vault-secrets-operator
helm install [RELEASE_NAME] .
```

The command deploys vault-secrets-operator on the Kubernetes cluster in the default configuration.
